...
class Vods:
    def __init__(self, vod_id, vod_date, vod_name, vod_link, channel_name, vod_lenght):
...
def vod_list_creater():
    while True:
        ...
        if len(dict(video_req)["videos"]) == 0:
            print("Vod list is empty,try again...")
            continue
        break
    ...
    for vod in dict_resp["videos"]:
        ...
        vod_list.append(Vods(vod_id, vod_date, vod_name, vod_link, channel_name, vod_lenght))
...
def vod_selector():
    ...
    cnt_vod = 0
    for vod_cnt in vod_list:
        cnt_vod += 1
        print(f"{cnt_vod}. {vod_cnt}")
